...
import { ShieldAlert, Shield, ArrowLeft, LogIn } from 'lucide-react';
...
export default function AdminRoute({ children }) {
        ...
        <div className="max-w-md w-full bg-card squircle-lg border border-border p-8 text-center space-y-6 shadow-sm">
          ...
          <div className="space-y-3 pt-2">
            <Link to={`/login?returnTo=${encodeURIComponent(location.pathname)}`} className="block w-full">
              <Button className="w-full h-12 bg-forest text-paper font-medium hover:bg-forest/90 flex items-center justify-center gap-2">
  ...
  // If logged in but role is not admin, show access restricted
  if (role !== 'admin') {
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-paper">
